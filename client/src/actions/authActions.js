...
//remove token local localStorage
localStorage.removeItem('jwtToken')
//remove auth header for future requests
//in utils folder - setAuthToken - if token is in local storage - it attaches token to header for every request
setAuthToken(false)

//set current user to an empty object which will also set isAuthenticated to false
dispatch(setCurrentUser({})) // see reducer - payload is empty object - thus will set isAuthenticated to false - sets back to initial state - loggin out
}
